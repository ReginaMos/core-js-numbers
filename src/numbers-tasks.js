...
function roundToPowerOfTen(num, pow) {
  ...
  const index = strNum.length - pow - 1;
  let res = strNum.slice(0, index);

  if (Number(strNum[index + 1]) >= 5) res += Number(strNum[index]) + 1;
  else res += strNum[index];
...
function isPowerOfTwo(num) {
  if (num > 0 && Math.log2(num) % 1 === 0) return true;
  return false;
}
